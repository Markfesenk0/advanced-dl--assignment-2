        ...
        for t in range(T_user - 1, -1, -1):
            ...
            for i in range(T - 1):
                if Alpha_bar[i] >= Gamma_bar[t] > Alpha_bar[i + 1]:
                    t_adapted = bisearch(
                        f=lambda _t: _log_cont_noise(_t, Beta[0].cpu().numpy(), Beta[-1].cpu().numpy(), T),
        ...
        for i, tau in enumerate(continuous_steps):
            ...
            if i == T_user - 1:  # the next step is to generate x_0
                ...
                sigma = kappa * torch.sqrt(
                    (1 - alpha_next) / (1 - Gamma_bar[T_user - 1 - i]) * (1 - Gamma_bar[T_user - 1 - i] / alpha_next))
...
def generate(output_name, model_path, model_config,
    ...
    try:
        ...
        net = torch.load(os.path.join(model_path))
        net = map_gpu(net)
        net.eval()
    ...
    torchvision.utils.save_image(images, os.path.join('./logs/', f'gen_samples_FastDPM.png'), nrow=batchsize)

    # save_image(make_grid(rescale(images)[:32]), fp=os.path.join('generated', '{}.jpg'.format(output_name)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-name', '--name', type=str,
                        choices=["cifar10", "lsun_bedroom", "lsun_church", "lsun_cat", "celeba64"],
                        help='Name of experiment')
    parser.add_argument('-ema', '--ema', action='store_true', help='Whether use ema')

    # fast generation parameters
    parser.add_argument('-approxdiff', '--approxdiff', type=str, choices=['STD', 'STEP', 'VAR'],
                        help='approximate diffusion process')
    ...
    parser.add_argument('-schedule', '--schedule', type=str, choices=['linear', 'quadratic'],
                        help='noise level schedules')

    # generation util
    parser.add_argument('-n', '--n_generate', type=int, default=25, help='Number of samples to generate')
    parser.add_argument('-bs', '--batchsize', type=int, default=5, help='Batchsize of generation')
    parser.add_argument('-gpu', '--gpu', type=str, default='cuda', choices=['cuda'] + [str(i) for i in range(16)],
                        help='gpu device')
    ...
    if n_exist < args.n_generate:
        ...
        model_path = os.path.join('./logs', 'ema_model.pt')
